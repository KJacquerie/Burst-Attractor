...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64},  idxPrePost::Vector{CartesianIndex{2}})
  ...
  Cpre_p=zeros(length(cpre_var),convert(Int64,T))
  Cpost_p=zeros(length(cpost_var),convert(Int64,T))
  Ca_p=zeros(length(ca_var),convert(Int64,T))
  ...
  gincr_plot = zeros(length(ww_var),convert(Int64,T))
  #event_plot = zeros(length(ww_var),convert(Int64,T))
  ...
  lncells::Array{Int64} = ones(ncellsC,1)
  ...
  for z = 1:Tdt
          ...
          for k=1:1:nPre
            ...
            if(state[z]<2) #testing phase > aucun changement de w
              ...
              # SLEEP
                  ...
                  if(ca_var[k,jj]>= theta_p)
                      ww_var[k,jj] +=(dt)*(1/tauw_p)*(Omega_p_sleep[k,jj] - ww_var[k,jj])
                  elseif(ca_var[k,jj]>=theta_d && ca_var[k,jj]<theta_p)
                      ww_var[k,jj] +=(dt)*(1/tauw_d)*(Omega_d - ww_var[k,jj])
                  else
                      ww_var[k,jj] +=(dt)*0.
            ...
            if state[z] <2
              if state[z] == 1 #|| state[z]==-1
                ...
                tot[k,jj] = z-TstartBurstTime[idx_burst]

                if ca_var[k,jj] <= theta_d
                    r[k,jj]+=1
                elseif ca_var[k,jj] > theta_d && ca_var[k,jj] <= theta_p
                    d[k,jj]+=1
                elseif ca_var[k,jj] > theta_p
                    p[k,jj]+=1
                end

                # CONTROL LOOP ON THE RESET VALUE
                    
                if(z== TstartBurstTime[idx_burst] + step )
                  w_HRCurr[k,jj] = predictW(r[k,jj],p[k,jj],d[k,jj],tot[k,jj],Omega_p_sleep[k,jj])
                  count_connections +=1
                end

                sigmoidValue[z] = sigmoidFunc(ww_var[k,jj], w_HRCurr[k,jj], slope)
                g_incr[k,jj] +=(dt)*(sigmoidFunc(ww_var[k,jj], w_HRCurr[k,jj], slope) - g_incr[k,jj])/tauINCR
                gCAMPA_var[k,jj] +=(dt)*g_incr[k,jj]*1/tauG

                if(count_connections>length(idxPrePost))
                  step += 1000/dt
                  count_connections =1
              ...
              # WAKE
                  ...
                  # ---- RETURN TO BASELINE IF WAKE STATE ----
                  g_incr[k,jj] +=(dt)*(0 - g_incr[k,jj])/tauINCR
                  step = 1000/dt
                  r[k,jj] = 0
                  d[k,jj] = 0
                  p[k,jj] = 0
    ...
    if(mod(z,1/dt)==0)
      ...
      #gincr_plot[:,idx_w]  = copy(reshape(g_incr, length(g_incr), 1))
      Cpre_p[:,idx_w] = copy(reshape(cpre_var, length(cpre_var), 1))
      Cpost_p[:,idx_w] = copy(reshape(cpost_var, length(cpost_var), 1))
      Ca_p[:,idx_w] = copy(reshape(ca_var, length(ca_var), 1))
  ...
  return  VV, Cpre_p, Cpost_p, Ca_p, ww, gCAMPA_plot
end
